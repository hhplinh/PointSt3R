...
def fast_reciprocal_NNs(pts1, pts2, subsample_or_initxy1=8, ret_xy=True, pixel_tol=0, ret_basin=False,
                        device='cuda', dense_grid=False, **matcher_kw):
    ...
    if dense_grid:
        ...
        max_iter = 10
    elif isinstance(subsample_or_initxy1, int) and pixel_tol == 0:
    ...
    if dense_grid:
        xy1_conv = np.unravel_index(xy1[converged], (H1, W1))
        xy2_conv = np.unravel_index(xy2[converged], (H2, W2))
        return xy1_conv[0].base[:, ::-1], xy2_conv[0].base[:, ::-1]
