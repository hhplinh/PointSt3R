...
from mast3r.datasets.utils.cropping import (extract_point_correspondences, gen_random_crops, in2d_rect, crop_to_homography)
...
class MASt3RBaseStereoViewDataset(BaseStereoViewDataset):
    def __init__(
        ...
        seed=None,
        use_soft_negs=False,
        only_dynamic_points=False,
        only_static_points=False,
        dyn_ratio=None
    ):
        ...
        self.use_soft_negs = use_soft_negs
        self.only_dynamic_points = only_dynamic_points
        self.only_static_points = only_static_points
    ...
    def generate_crops_from_pair(self, view1, view2, resolution, aug_crop_arg, n_crops=4, rng=np.random):
        ...
        for i in range(2):
            ...
            if "dynamic_matches" in view:
                mapping, dynamic_matches = np.unique(view["dynamic_matches"], return_inverse=True)
                dynamic_matches = dynamic_matches.astype(np.uint32).reshape(view["dynamic_matches"].shape)
                res = np.array(Image.fromarray(dynamic_matches).transform(imsize, Image.Transform.PERSPECTIVE, homo8))
                view["dynamic_matches"] = mapping[res]

            if "dynamic_mask" in view:
                mapping, dynamic_mask = np.unique(view["dynamic_mask"], return_inverse=True)
                dynamic_mask = dynamic_mask.astype(np.uint32).reshape(view["dynamic_mask"].shape)
                res = np.array(Image.fromarray(dynamic_mask).transform(imsize, Image.Transform.PERSPECTIVE, homo8))
                view["dynamic_mask"] = mapping[res]

                mapping, dynamic_mask_valids = np.unique(view["dynamic_mask_valids"], return_inverse=True)
                dynamic_mask_valids = dynamic_mask_valids.astype(np.uint32).reshape(view["dynamic_mask_valids"].shape)
                res = np.array(Image.fromarray(dynamic_mask_valids).transform(imsize, Image.Transform.PERSPECTIVE, homo8))
                view["dynamic_mask_valids"] = mapping[res]

            if "vis_in_other" in view:
                mapping, vis_in_other = np.unique(view["vis_in_other"], return_inverse=True)
                vis_in_other = vis_in_other.astype(np.uint32).reshape(view["vis_in_other"].shape)
                res = np.array(Image.fromarray(vis_in_other).transform(imsize, Image.Transform.PERSPECTIVE, homo8))
                view["vis_in_other"] = mapping[res]

                mapping, vis_in_other_valids = np.unique(view["vis_in_other_valids"], return_inverse=True)
                vis_in_other_valids = vis_in_other_valids.astype(np.uint32).reshape(view["vis_in_other_valids"].shape)
                res = np.array(Image.fromarray(vis_in_other_valids).transform(imsize, Image.Transform.PERSPECTIVE, homo8))
                view["vis_in_other_valids"] = mapping[res]
    ...
    def __getitem__(self, idx):
        ...
        if self.n_corres > 0 and ('corres' not in view):
            ...
            corres1, corres2, valid, dynamic_corres = extract_point_correspondences(
                *views, self.n_corres, self._rng,
                nneg=self.nneg, only_dynamic_points=self.only_dynamic_points,
                only_static_points=self.only_static_points, dyn_ratio=self.dyn_ratio
            )
