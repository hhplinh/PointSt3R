...
class Cat_MLP_LocalFeatures_DPT_Pts3d(PixelwiseTaskWithDPT):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.add_mask_head = add_mask_head
        if self.add_mask_head:
            self.head_dynamic_mask = Mlp(
                in_features=idim,
                hidden_features=int(hidden_dim_factor * idim),
                out_features=(1 * self.patch_size**2)
            )

        self.add_vis_head = add_vis_head
        if self.add_vis_head:
            self.head_vis = Mlp(
                in_features=idim,
                hidden_features=int(hidden_dim_factor * idim),
                out_features=(1 * self.patch_size**2)
    ...
    def forward(self, decout, img_shape):
        ...
        # Extract dynamic_mask if added
        if self.add_mask_head:
            dynamic_mask_out = self.head_dynamic_mask(cat_output)
            dynamic_mask_out = dynamic_mask_out.transpose(-1, -2).view(B, -1, H // self.patch_size, W // self.patch_size)
            dynamic_mask_out = F.pixel_shuffle(dynamic_mask_out, self.patch_size)  # B,d,H,W
            dynamic_mask_out = dynamic_mask_out.permute(0, 2, 3, 1).reshape(B, H, W) # B, H, W, 1
            out["dynamic_mask"] = dynamic_mask_out

        # Extract visibility if added
        if self.add_vis_head:
            vis_out = self.head_vis(cat_output)
            vis_out = vis_out.transpose(-1, -2).view(B, -1, H // self.patch_size, W // self.patch_size)
            vis_out = F.pixel_shuffle(vis_out, self.patch_size)  # B,d,H,W
            vis_out = vis_out.permute(0, 2, 3, 1).reshape(B, H, W) # B, H, W, 1
            out["vis_in_other"] = vis_out
        return out
...
def mast3r_head_factory(head_type, output_mode, net, has_conf=False):
    ...
    if head_type == 'catmlp+dpt' and output_mode.startswith('pts3d+desc'):
        output_mode_split = output_mode.split("+")
        add_mask_head = True if "mask" in output_mode_split else False
        add_vis_head = True if "vis" in output_mode_split else False
        local_feat_dim = int(output_mode.split("desc")[-1].split("+")[0])
        ...
        return Cat_MLP_LocalFeatures_DPT_Pts3d(
            net, local_feat_dim=local_feat_dim, has_conf=has_conf,
            ...
            head_type='regression',
            add_mask_head=add_mask_head,
            add_vis_head=add_vis_head
        )
    else:
