...
def get_dummy_loss(model):
    dummy_loss = None
    for p in model.parameters():
        if p.requires_grad:
            #return (p.sum() * 0.0)
            dummy = (p * 0.0).sum()
            dummy_loss = dummy if dummy_loss is None else dummy_loss + dummy
    if dummy_loss is None:
        raise RuntimeError("No model parameters require grad!")
    return dummy_loss
